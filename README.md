...
The infrastructure utilizes decoupled compute services communicating via message queues and persistent WebSocket tunnels to ensure low-latency state synchronization without risking main thread bottlenecking.

**Data Evaluation Flow:**
`React Frontend (Student)` ➔ `Nginx Proxy` ➔ `Express API (Producer)` ➔ `Redis Queue` ➔ `Worker Node (Consumer)` ➔ `Dockerized C++ Executor` ➔ `Socket.io Broadcast` ➔ `React Frontend (Teacher Dashboard or Solo Client)`
...
2. **Producer-Consumer Offloading**: To prevent the primary API from bottlenecking during concurrent submissions, the Express server acts as a producer. It validates the request payload and offloads it into a Redis-backed asynchronous message queue.
3. **Remote Code Execution (RCE) & Sandboxing**: Independent worker nodes act as consumers, continually polling the Redis queue. They dequeue tasks and securely compile/execute the C++ code within isolated Docker containers acting as ephemeral sandboxes. Resource limits (Time Limit Exceeded/TLE and Memory Limit Exceeded/MLE) are strictly enforced by the container to prevent infinite loops and malicious scripts from crashing the host server.
...
- **Dual Execution Modes**: 
  ...
  - **Host Room**: Stateful, WebSocket-based classroom environments tracked persistently via MongoDB, allowing educators to generate secure sessions for students to join.
...
- **Producer-Consumer Message Queuing**: Utilizes Redis to distribute heavy compilation workloads asynchronously, mitigating API blocking under concurrent classroom load.
...
| **State & Queuing Pipeline** | Redis, Socket.io |
...
Ensure the local host has active daemon processes for MongoDB, Redis, and Docker.
```bash
...
# Note: Point this to your specific worker script entry point
node src/worker.js 
```
...
CodeSpace utilizes a distributed deployment architecture mapping compute services to optimized runtime environments.
...
- **Compute Instance**: The Node.js Express API, the Redis job queue, and isolated Dockerized C++ worker nodes are deployed on an **AWS EC2 (Ubuntu)** server.
- **Process Management**: Both the Express API and the Redis execution workers are managed via **PM2**. This daemonizes the processes, ensuring automated restarts and log aggregation for high availability.
- **Reverse Proxy**: **Nginx** operates on the EC2 instance as the primary internet-facing proxy. It handles:
  ...
  - **WebSocket Upgrades**: Nginx explicitly intercepts Socket.io HTTP polling sequences and injects `Connection: Upgrade` and `Upgrade: websocket` headers. This reliably transitions standard HTTP connections into permanent, bidirectional `wss://` WebSocket tunnels bridging the client and the AWS EC2 instance.
